...
class Path():
    def __init__(self):
        ...
        self.current_pose_sub = rospy.Subscriber(
            '/model_pose', PoseStamped, self.current_pose_cb)
        self.goal_status_sub = rospy.Subscriber(
            '/gopher/move_base/status', GoalStatusArray, self.goal_status_cb)
    ...
    def goal_status_cb(self, msg):
        ...
        if len(msg.status_list) > 0:
            if msg.status_list[-1].status == 3:
                if self.status == 1:
                    print("Goal reached. The total path length is: ",
                          self.path_length)
                self.status = 3
            ...
            if self.status == 1:
                ...
                self.path_length += self.get_distance(
                    self.robot_pose_current, self.robot_pose_previous)
                self.robot_pose_previous = self.robot_pose_current
